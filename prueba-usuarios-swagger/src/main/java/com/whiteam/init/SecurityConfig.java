...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests()
				...
				.antMatchers(HttpMethod.GET, "/showall").hasRole("USER").antMatchers("/showall").authenticated()
		...
		// solo los miembros del rol admin podrán realizar altas
		// y para acceder la lista de cursos, tendrán que estar autenticados
		.antMatchers(HttpMethod.GET, "/curso").hasRole("ADMIN").antMatchers("/cursosDuracion/{duracion}").authenticated()
		// .antMatchers("/curso/**").authenticated()
		.and().httpBasic();
	}
